...
export async function POST(request: Request) {
  ...
  if (!image_input || !Array.isArray(image_input) || image_input.length === 0) {
    return NextResponse.json(
      { detail: "image_input array is required for image-to-image task" },
      { status: 400 }
    );
  }
  ...
  // 准备 Seedream 4.0 API 请求参数（包含 image_input 参数）
  const input: any = {
    prompt,
    image_input, // 图生图的关键参数，1-10张图片数组
    size,
    aspect_ratio,
  ...
  try {
    // 调用云雾 API 创建任务（同一个端点，有 image 就是图生图）
    ...
    console.error("Seedream image-to-image API error:", error);
    return NextResponse.json(
